...
export default function UpcomingExams() {
  ...
  const fetchTimetables = async () => {
    ...
    try {
      ...
      // show first timetable by default
      if (res.data.length > 0) {
        setTimetable(res.data[0].timetable);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const handleCourseSelect = (courseId) => {
    ...
    const filtered = allTimetables.filter((t) => t.course?._id === courseId);

    if (filtered.length > 0) {
      setTimetable(filtered[0].timetable);
    } else {
      setTimetable([]);
    }
  };
      ...
      <div className="max-w-5xl mt-[90px] ml-[300px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <select
              className="w-full bg-slate-50 border border-slate-200 text-sm rounded-xl px-4 py-3"
              value={selectedCourse}
              onChange={(e) => handleCourseSelect(e.target.value)}
            >
              <option value="">— Choose a course —</option>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
            ...
            <select
              className="w-full bg-slate-50 border border-slate-200 text-sm rounded-xl px-4 py-3 disabled:opacity-40"
              value={selectedSemester}
              onChange={(e) => handleSemesterSelect(e.target.value)}
              disabled={!selectedCourse}
            >
        ...
        {!loading && timetable.length > 0 && (
          <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b bg-slate-50">
              <h2 className="text-base font-bold">Exam Schedule</h2>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 text-xs uppercase border-b">
                    ...
                    <th className="px-5 py-3 text-left">Exam Date</th>
                ...
                <tbody>
                  {timetable.map((item, index) => (
                    <tr key={index} className="border-b hover:bg-slate-50">
                      ...
                      <td className="px-5 py-4">{formatTime(item.time)}</td>
